...
struct DistanceNode{
    cell_t cell;
    float distanceInCellFromObstacle;
    bool operator<(const DistanceNode rhs) const{
        return rhs.distanceInCellFromObstacle < distanceInCellFromObstacle;
    }
    DistanceNode(cell_t cell, int distanceInCellFromObstacle)
    :cell(cell)
    ,distanceInCellFromObstacle(distanceInCellFromObstacle)
    {}
};
class OccupancyGrid;
...
public:
    ...
    */
    ...
    void initializeDestances(const OccupancyGrid& map);
