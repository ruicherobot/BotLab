...
bool check_valid(const MotionPlanner& planner, pose_xyt_t pose, pose_xyt_t curr_pose) {
    if (planner.isValidGoal(pose)) {
        robot_path_t temp_path = planner.planPath(curr_pose, pose);
        if(temp_path.path_length <= 2){
            return false;
        }
        else if(planner.isPathSafe(temp_path)){
            return true;
        }
        else{
            return false;
                                   ...
                                   const MotionPlanner& planner)
{
    ...
    std::cout<<"clX  "<<closest_point.x<<"  clY  "<<closest_point.y<<" \n";
    std::cout<<"rbX  "<<robotPose.x<<"  rbY  "<<robotPose.y<<" \n";
    ...
    pose_xyt_t search_pose;
    for (int i = 0; i <= 8; i++) {
        search_pose.x = closest_point.x + xDeltas[8] * square_radius;
        search_pose.y = closest_point.y + yDeltas[8] * square_radius;
        if (check_valid(planner, search_pose, robotPose)){
            foundPose = true;
            goal_pose = search_pose;
            std::cout<<"valid(planner, search_pose, robotPose)\n";
        }
        
    }
    
    // std::cout << "planPath(robotPose, goal_pose)!\n";
